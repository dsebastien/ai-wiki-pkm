...
sources:
  ...
  - Brian Potter, Construction Physics — engineering writings on continuous-flow production
review_count: 0
review_interval:
last_reviewed:
notes:
...
1. **Redefine success metrics.** Replace vanity metrics (subscribers, views) with *evergreen notes created daily*. The latter is a process metric, not an outcome metric; it is in your control.
2. **Build privately first.** Maintain the pipeline for 6+ months before publishing. Continuous flow needs time to establish before it can support external output.
3. **Create atomic note systems.** Organize around ideas (not people/dates), so outputs can be composed from reusable units.
4. **Embrace continuous observation.** Capture inputs from all life contexts, not just desk work. The pipeline's mouth is wider than scheduled reading.

## Contrast with Common PKM Antipatterns

Continuous transformation names several PKM antipatterns it opposes:

- **Collector's fallacy** — input without processing ([[AI Wiki - PKM - Collector's Fallacy]])
...
- **[[AI Wiki - PKM - Inbox Zero for Notes|Inbox zero for notes]]** — a continuous-flow discipline for the input buffer
- **[[AI Wiki - PKM - Creativity Flywheel|Creativity flywheel]]** — compatible and complementary; the flywheel describes the shape (momentum), continuous transformation describes the mechanism (flow)
- **[[AI Wiki - PKM - Creation-to-Consumption Ratio|Creation-to-consumption ratio]]** — the balance across the pipeline's two ends
- **[[AI Wiki - PKM - Knowledge Funnel|Knowledge funnel]]** — the staged pipeline view
- **[[AI Wiki - PKM - Evergreen Notes|Evergreen notes]]** — the continuous-flow output artifact
- **[[AI Wiki - PKM - Slow Burn Method|Slow burn method]]** — compatible at longer timescales
- **[[AI Wiki - PKM - Dump Lump Jump|Dump, lump, jump]]** — a specific continuous-transformation pattern for individual notes
